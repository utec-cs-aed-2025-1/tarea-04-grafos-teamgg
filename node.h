...
//
// Variables miembro
//     - id            : Identificador de un vertice, debe ser irrepetible entre
//     vertices
//     - coord         : La coordenada donde se encuentra el vertice (usado por
//     SFML)
...
struct Node {
  ...
  static void parse_csv(const std::string &nodes_path,
                        std::map<std::size_t, Node *> &nodes) {
    std::ifstream file(nodes_path);
    if (!file.is_open()) {
      throw std::runtime_error("Could not open nodes file: " + nodes_path);
    }
    ...
    while (true) {
      ...
      nodes.insert(
          {identifier, new Node(identifier, std::stof(y), std::stof(x))});
