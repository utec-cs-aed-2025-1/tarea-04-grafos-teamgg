...
class GUI {
  ...
  // 1NN es un algoritmo muy popular que retorna el 1 Nearest Neighbour (de ahí
  // el nombre 1NN), o vecino más cercano de una coleccion de elementos a una
  // query dada. En este caso, nos interesa conocer cuál es el nodo mas cercano
  // al punto 'query' pasado como parámetro.
...
public:
  ...
  void main_loop() {
    ...
    while (window_manager.is_open()) {
      // Verifica los eventos de la ventana que pueden ser 'triggereados'
      // (lanzados) por el usuario en la iteración actual
      while (window_manager.poll_event(event)) {
        ...
        case sf::Event::KeyPressed: {
          ...
          // E = Extra flag. Si es verdadero, hace un display de todos los
          // 'edges'
        ...
        case sf::Event::MouseButtonPressed: {
          ...
          if (path_finding_manager.src == nullptr) {
            // Encuentra el vértice más cercano a la posición del mouse y asigna
            // el vértice a 'src'
            path_finding_manager.src = _1NN(graph.nodes, mouse_position);
            if (path_finding_manager.src) {
      ...
      // si 'extra_lines' es true, también dibujará el resto de aristas
      // visitadas
