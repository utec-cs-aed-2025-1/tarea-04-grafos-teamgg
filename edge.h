...
//     - max_speed     : Velocidad maxima en la que se puede ir de 'src' a
//     'dest'
//     - length        : Longitud de la trayectoria de 'src' a 'dest'
//     - one_way       : Si es falso, significa que tambien existe una arista en
//     el grafo de 'dest' a 'src'.
//                       Caso contrario, significa que solo existe la arista
//                       actual de 'src' a 'dest'
//     - lanes         : La cantidad de carriles en el camino de 'src' a 'dest'
//     - color         : El color de la linea que conecta a 'src' y 'dest', es
//     usado por SFML
//     - thickness     : El grosor de la linea que conecta a 'src' y 'dest', es
//     usado por SFML
//
// Funciones miembro
...
struct Edge {
  ...
  explicit Edge(Node *src, Node *dest, int max_speed, double length,
                bool one_way, int lanes)
      : max_speed(max_speed), length(length), one_way(one_way), lanes(lanes),
        src(src), dest(dest) {}
  ...
  static void parse_csv(const std::string &edges_path,
    ...
    if (!file.is_open()) {
      throw std::runtime_error("No se pudo abrir: " + edges_path);
    }
    ...
    while (true) {
      ...
      Edge *edge =
          new Edge(src_node, dest_node, std::stoi(max_speed), std::stod(length),
